                  ...
                  <div class="global-card">
                    <div class="d-flex flex-wrap justify-content-between align-items-center">
                        ...
                        @if ($myTicket->status != 3 && $myTicket->user)
                            <button class="btn btn--base close-button mb-2 confirmationBtn " type="button"
                    ...
                    @if ($myTicket->status != 4)
                        <form method="post" action="{{ route('ticket.reply', $myTicket->id) }}"
                            ...
                            <div class="text-end mt-4">
                                <a href="javascript:void(0)" class="btn btn--base addFile"><i
                                        class="fa fa-plus"></i> @lang('Add New')</a>
                            </div>
                            <div class="form-group">
                                <label class="form-label">@lang('Attachments')</label> <small
                                    class="text--base">@lang('Max 5 files can be uploaded'). @lang('Maximum upload size is')
                            ...
                            <button type="submit" class="btn btn--base mt-2"> <i class="fa fa-reply"></i>
                                @lang('Reply')</button>
                        </form>
                    @endif
                    ...
                    <div class="mt-4">
                        @foreach ($messages as $message)
                            @if ($message->admin_id == 0)
                                <div class="row color-whitw-wrap border border--base border-radius-3 my-3 py-3 mx-2">
                                            ...
                                            <div class="mt-2">
                                                @foreach ($message->attachments as $k => $image)
                                                    <a href="{{ route('ticket.download', encrypt($image->id)) }}"
                                                        class="me-3 text--base"><i class="fa fa-file"></i> @lang('Attachment')
                                                        {{ ++$k }} </a>
                                                @endforeach
                                            </div>
                                        @endif
                            ...
                            @else
                                <div class="row border border-warning border-radius-3 my-3 py-3 mx-2">
                                    ...
                                    <div class="col-md-9">
                                        <p class="text-muted fw-bold my-3 text-white">
                                            @lang('Posted on') {{ $message->created_at->format('l, dS F Y @ H:i') }}</p>
