...
<div class="row">
    <div class="col-md-12">
        <form action="{{ route('user.review.submit') }}" method="post">
            @csrf
            <h3>@lang('Edit Review for') <a class="text--primary"
                    href="{{ route('product', ['slug' => slug($review->product->title), 'id' => $review->product->id])}}">{{
                    __($review->product->title) }}</a></h3>
            ...
            <textarea class="form-control form--control  mb-3" name="review" id="" cols="30"
