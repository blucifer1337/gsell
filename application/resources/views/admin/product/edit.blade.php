                ...
                <div class="card-body">
                    <form action="{{ route('admin.product.update', $product->id) }}" method="post"
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control bmd-input" name="title" value="{{ $product->title }}"
                                required>
                        </div>
                        <div class="form-group">
                            ...
                            <select name="device_id" class="form-control" required>
                                ...
                                @foreach ($devices as $device)
                                    <option value="{{ $device->id }}"
                                        {{ $device->id == $product->device_id ? 'selected' : null }}>
                                        {{ $device->name }}</option>
                                @endforeach
                            </select>
                        </div>
                            ...
                            <select name="platform_id" class="form-control" required>
                                ...
                                @foreach ($platforms as $platform)
                                    <option value="{{ $platform->id }}"
                                        {{ $platform->id == $product->platform_id ? 'selected' : null }}>
                            ...
                            <select name="genre_id" class="form-control">
                                ...
                                @foreach ($genres as $genre)
                                    <option value="{{ $genre->id }}"
                                        {{ $genre->id == $product->genre_id ? 'selected' : null }}>
                                        {{ $genre->name }}</option>
                                @endforeach
                            </select>
                        </div>
                                ...
                                <div class="price-input-section">
                                    <div class="row mb-3">
                                        <div class="col-lg-10 col-md-10">
                                            <p>@lang('Uplod Product Images')</p>
                                        </div>
                                        <div class="col-lg-2 col-md-2 text-end">
                                            <button type="button" class="btn btn--base addmoreImageNew ms-0 mt-2"><i
                                        ...
                                        <div class="col-lg-12">
                                            <div class="addmoreImageContainer">
                                                <div class="row">
                                                                    ...
                                                                    <div class="thumb newthumb">
                                                                        <div class="avatar-preview">
                                                                            <div class="profilePicPreview newprofilePicPreview"
                                                                                style="background-image: url({{ getImage(getFilePath('product') . '/' . $item->image, getFileSize('product')) }})">
                                                                                <button type="button"
                                                    ...
                                                    @empty
                                                        <div class="col-lg-12">
                                                            <p class="mt-3">@lang('No image found')</p>
                                                        </div>
                                                    @endforelse
                                                </div>
                                            </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="trumEdit" name="description" id="" cols="30" rows="10" required>{{ $product->description }}</textarea>
                        </div>
                        <div class="form-group">
                            <label>@lang('Short Description')</label>
                            <textarea class="trumEdit" name="short_description" id="dd" cols="30" rows="10" required>{{ $product->short_description }}</textarea>
                        </div>
                        <div class="row">
                            <div class="col-lg-6">
                                <label>@lang('Minimum System Requirements')</label>
                                <textarea class="trumEdit" name="minimum" id="" cols="30" rows="10">{{ $product->minimum }}</textarea>
                            </div>
                            <div class="col-lg-6">
                                <label>@lang('Recommended System Requirements')</label>
                                <textarea class="trumEdit" name="recommended" id="" cols="30" rows="10">{{ $product->recommended }}</textarea>
                            </div>
                        </div>
                        <div class="row my-3">
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <input class="form-control bmd-input" name="price" type="number"
                                        value="{{ showAmount(@$product->price, 2) }}" step="any" required>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>@lang('Discount') %</label>
                                    <input class="form-control bmd-input" name="discount"
                                        value="{{ $product->discount }}" type="number" step="any">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>@lang('Version') </label>
                                    <input class="form-control bmd-input" name="version" value="{{ $product->version }}"
                            ...
                            <div class="col-md-6">
                                <div class="flex-grow-1">
                                    ...
                                    <select name="is_trending" class="form-control" required>
                                        <option value="0" {{ $product->is_trending == 0 ? 'selected' : null }}>
                                            @lang('No')
                                        </option>
                                        <option value="1" {{ $product->is_trending == 1 ? 'selected' : null }}>
                                            @lang('Yes')
                                        </option>
                                    </select>
                                </div>
                            ...
                            <div class="col-md-6">
                                <div class="flex-grow-1">
                                    ...
                                    <select name="status" class="form-control" required>
                                        <option {{ $product->status == 1 ? 'selected' : null }} value="1">
                                            @lang('Active')
                                        </option>
                                        <option {{ $product->status == 0 ? 'selected' : null }} value="0">
                                            @lang('Inactive')
                                        </option>
                                    </select>
                                </div>
                        ...
                        <div class="form-group">
                            <div class="image-upload">
                                <div class="thumb newthumb">
                                    ...
                                    <div class="avatar-edit">
                                        <input type="file" class="profilePicUpload" name="images[${inputId}]" id="${inputId}" accept=".png, .jpg, .jpeg">
