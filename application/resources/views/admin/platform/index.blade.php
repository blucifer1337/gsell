                    ...
                    <table class="table table--light style--two">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>{{ $platform->name }}</td>
                                <td>
                                    @php echo __($platform->icon) @endphp
                                </td>
                                <td>
                                    @if($platform->is_menu_item == 1)
                                    <span class="badge badge--success">@lang('Yes')</span>
                                    @else
                                    <span class="badge badge--warning">@lang('No')</span>
                                    @endif
                                </td>
                                <td>{{ showDateTime($platform->created_at) }}</td>
                                <td>
                                    <button title="@lang('Edit')"
                                        class="btn btn-sm btn--warning"
                                        data-bs-toggle="modal" data-bs-target="#editModal"
                                        data-object="{{ $platform }}"
                                        data-action="{{ route('admin.platform.update', $platform->id) }}">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">@lang('Add Platform')</h5>
...
<div class="modal fade" id="editModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">@lang('Edit Platform')</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
